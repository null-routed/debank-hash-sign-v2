...
func GenerateSignature(requestMethod string, path string, parameters string) (string, int64, string, error) {
    ...
    randStr := fmt.Sprintf("debank-api\n%s\n%d", nonce, timestamp)
    randStrHash := Sha256(randStr)

    requestParams := fmt.Sprintf("%s\n%s\n%s", strings.ToUpper(requestMethod), strings.ToLower(path), strings.ToLower(parameters))
    requestParamsHash := Sha256(requestParams)

    signature := HmacSha256([]byte(randStrHash), []byte(requestParamsHash))
